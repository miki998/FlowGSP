...
from gyraph.utils import np, nx, matplotlib, plt, colors, hermitian, Optional, cm
...
class Graph:
    ...
    def __init__(
        self, G=None, adj_matrix=None, pos=None, draw: bool = True, debug: bool = True
    ):
        if G is not None:
            self.G = G
            ...
            raise ValueError(
                "Either a graph (G) or an adjacency matrix (adj_matrix) must be provided."
        ...
        self.debug = debug  # Placeholder for enabling/disabling debug prints
        self.to_draw = draw  # Placeholder for enabling/disabling drawing

    def from_adjacency_matrix(self, adj_matrix: np.ndarray):
    ...
    def set_operator(self, name: str = "adjacency", **kwargs):
        """
        Returns the operator associated with the graph.
        """
        if self.adj_matrix is None:
            raise ValueError(
                "Adjacency matrix is not initialized. Please provide a valid adjacency matrix or a Graph."
            )
        ...
        elif name == "time_vertex_laplacian":
            self.operator = TimeVertexLaplacian(self, **kwargs)
        elif name == "time_vertex_adjacency":
            self.operator = TimeVertexAdjacency(self, **kwargs)
        else:
            raise ValueError(
                f"Unknown operator name: {name} \
                             (must be one of ['adjacency', 'laplacian', 'advection_diffusion', 'time_vertex_laplacian'\
    ...
    def draw(
        ...
        arrow_size: int = 10,
        arrow_width: int = 2,
        symmetric_color="tab:gray",
        asymmetric_color="tab:red",
        **kwds,
    ):
        """
        ...
        """
        if not self.to_draw:
            print("Drawing is disabled for this graph.")
            return
        if axes is None:
        ...
        nx.draw_networkx_edges(
            self.G,
            pos=self.pos,
            edgelist=list(symmetric_edges),
            ax=axes,
            edge_color=symmetric_color,
            arrows=False,
            width=arrow_width,
        )
        ...
        nx.draw_networkx_edges(
            self.G,
            pos=self.pos,
            edgelist=list(asymmetric_edges),
            ax=axes,
            edge_color=asymmetric_color,
            arrows=True,
            connectionstyle="arc3,rad=0.0",
            arrowsize=arrow_size,
            width=arrow_width,
            min_source_margin=0.01,
    ...
    def draw_signal(
        ...
        **kwds,
        ...
        if np.allclose(signal, 0):
            print(
                "Signal is all zeros, plotting graph with default node size and color."
            )
            signal = np.ones(self.N)
        ...
        if cmap is None:
            ...
            node_color = [
                cmap(normalized_values[k]) for k in range(len(normalized_values))
            ]
        ...
        if nodetype == "color":
            ...
            nx.draw_networkx_nodes(
                self.G, pos=self.pos, node_color=signal, ax=axes, **kwds
            )
    ...
    def assymetry_level(self) -> float:
        """
        Calculate the assymetry level of the graph.
        """
        ratio_asymmetric = np.linalg.norm(
            self.adj_matrix - self.adj_matrix.T
        ) / np.linalg.norm(self.adj_matrix)
        return ratio_asymmetric

    def assymetry_edge_level(
        self, return_number: bool = False, verbose: bool = False
    ) -> float:
        """
        ...
        """
        nb_symmetric = (
            np.sum((self.adj_matrix + self.adj_matrix.T) == 2) // 2
        )  # divide by 2 because we consider 1 edge even though its 2 entries
        nb_assymetric = (
            np.sum((self.adj_matrix + self.adj_matrix.T) == 1) // 2
        )  # this is re-checked
        if verbose:
            print(
                f"Number of symmetric edges: {nb_symmetric}, Number of asymmetric edges: {nb_assymetric}"
            )
        if return_number:
            return nb_assymetric / (nb_symmetric + nb_assymetric) if (
                nb_symmetric + nb_assymetric
            ) > 0 else 0, (nb_symmetric, nb_assymetric)
        return (
            nb_assymetric / (nb_symmetric + nb_assymetric)
            if (nb_symmetric + nb_assymetric) > 0
            else 0
        )

    def degree_entropy(self, degree_type: str = "in") -> float:
        """
        ...
        if degree_type == "in":
            ...
            p = (
                in_degree / np.sum(in_degree)
                if np.sum(in_degree) > 0
                else np.zeros_like(in_degree)
            )
        else:
            out_degree = np.sum(self.adj_matrix, axis=0)
            p = (
                out_degree / np.sum(out_degree)
                if np.sum(out_degree) > 0
                else np.zeros_like(out_degree)
            )
