...
def p_value(
    null_distrib: np.ndarray, statistic: float, two_tail: bool = False
) -> float:
    """
...
def circular_stats(
    alpha_deg: np.ndarray, weights: np.ndarray = None
) -> Tuple[float, float]:
    """
...
def circular_correlation(
    alpha_deg: np.ndarray, beta_deg: np.ndarray, weights: np.ndarray = None
) -> float:
    """
...
def sample_circular_complex_gaussian(
    mean: np.ndarray, covariance: np.ndarray, n_samples: int = 1, seed: int = None
) -> np.ndarray:
    """
