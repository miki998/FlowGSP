...
from gyraph.utils import np, deepcopy, Optional
...
def find_best_pair(
    ...
    vl: np.ndarray,
    vr: np.ndarray,
    prefer_mask: Optional[np.ndarray],
    opt_eps: float = 1e-5,
) -> np.ndarray:
    """
    ...
    if prefer_mask is not None:
        ...
        best_pairs = np.asarray(
            np.where(np.abs(cross_values) > opt_eps)
        ).T  # Use different from 0 criterion
    else:
        cross_values = cross_values * mask
        best_pairs = np.asarray(
            np.where(np.abs(cross_values - cross_values.max()) < 1e-5)
        ).T  # Use maximum argument criterion

    return best_pairs


def destroy_jordan_blocks(
    A: np.ndarray,
    prefer_nodes: list = [],
    ...
    while np.linalg.matrix_rank(vr) < n:
        ...
        dot = np.abs(vr.T @ vr)
        dot[dot > 1.0] = 1.0  # Numerical stability
        D = np.nan_to_num(np.arccos(dot))
...
def destroy_jordan_blocks_laplacian(
    A: np.ndarray, prefer_nodes: list = [], max_iter: int = 10000
) -> np.ndarray:
    """
    Destroy the Jordan blocks in the input matrix A by setting the smallest entries to 1.
...
def destroy_zero_eigenvals(
    ...
    while np.min(np.abs(D)) < eps:
        ...
        if repeat_count >= 10:
            if verbose:
                print("Remove preferential nodes")
            both, one = [], []
            none = find_best_pair(ret, np.argmin(np.abs(D)), vl, vr, None)
        else:
            both = [
                find_best_pair(ret, possible_index[k], vl, vr, mask_both)
                for k in range(len(possible_index))
            ]
            both = np.concatenate(both)
            one = [
                find_best_pair(ret, possible_index[k], vl, vr, mask_one)
                for k in range(len(possible_index))
            ]
            one = np.concatenate(one)
            none = [
                find_best_pair(ret, possible_index[k], vl, vr, None)
                for k in range(len(possible_index))
            ]
