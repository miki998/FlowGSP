...
def dirichlet(signal: np.ndarray, L: np.ndarray, normalize: bool = True) -> float:
    """
...
def sobolev(
    signal: np.ndarray, L: np.ndarray, norm: str = "L2", normalize: bool = False
) -> float:
    """
...
def directed_variation(
    signal: np.ndarray, A: np.ndarray, normalize: bool = False
) -> float:
    """
    ...
    """
    assert (
        signal.shape[0] == A.shape[0]
    ), "Signal s and adjacency matrix A must have the same number of nodes"
