...
importing the full gyraph package, which has dependencies (torch, etc.)
...
def load_logging_config():
    ...
    spec = importlib.util.spec_from_file_location(
        "logging_config",
        Path(__file__).parent.parent / "gyraph" / "utils" / "logging_config.py",
    )
...
def load_logging_utils():
    ...
    sys.modules["gyraph.utils.logging_config"] = logging_config
    ...
    def test_setup_logger_with_file(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            logger = self.logging_config.setup_logger(
                "test_logger", level="WARNING", log_file=log_file
            )
    ...
    def test_configure_experiment_logging(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            logger = self.logging_config.configure_experiment_logging(
                experiment_name="test_exp", verbose=False, results_dir=tmpdir
            )
...
class TestLoggingUtils(unittest.TestCase):
    ...
    def test_setup_experiment_logger(self):
        ...
        logger = self.logging_utils.setup_experiment_logger(
            "test_exp", verbose=False, log_to_file=False
        )
        self.assertIsNotNone(logger)
    ...
    def test_setup_experiment_logger_with_file(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            logger = self.logging_utils.setup_experiment_logger(
                "test_exp", verbose=False, log_to_file=True, results_dir=tmpdir
            )
    ...
    def test_print_section(self):
        ...
        logging_config = load_logging_config()
        logger = logging_config.setup_logger("test", level="WARNING")
